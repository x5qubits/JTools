...
namespace JTcpNetwork
{
    public class NetworkWriter
    {
        ...
        public void Write(byte[] buffer, int count)
        {
            ...
            {
                if (NetConfig.logFilter >= LogFilter.Error) { Log.Error("Write: buffer is too large (" + count + ") bytes. The maximum buffer size is 64K bytes."); }
                return;
            }
        ...
        public void WriteBytesAndSize(byte[] buffer, int count)
        {
            ...
            if (count > UInt16.MaxValue)
            {
                if (NetConfig.logFilter >= LogFilter.Error) { Log.Error("WriteBytesAndSize: buffer is too large (" + count + ") bytes. The maximum buffer size is 64K bytes."); }
                return;
            }
        ...
        public void WriteBytesFull(byte[] buffer)
        {
            ...
            {
                if (NetConfig.logFilter >= LogFilter.Error) { Log.Error("WriteBytes: buffer is too large (" + buffer.Length + ") bytes. The maximum buffer size is 64K bytes."); }
                return;
            }
